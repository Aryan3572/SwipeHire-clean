...
export const getJobFeed = async (req, res) => {
  try {
    ...
    const response = await axios.get(JSEARCH_URL, {
      params: {
        query: `${userSkills.join(" ")} developer`,
        num_pages: 1,
      },
        headers: {
    ...
    const scoredJobs = jobs.map(job => {
      ...
      const matched = userSkills.filter(s => jobSkills.includes(s.toLowerCase()));
      const score = Math.floor((matched.length / userSkills.length) * 100) || 10;
